# 08-Plates archive — the governing docs before s47, verbatim

At s47 the four files below were replaced by `../README.md`, `../canon-brief.md` and `../prompt-system.md` (rules only) and `../scene-tests.md` (the specimens). Each is held here exactly as it stood, so every finding, run record, revision and ruling it carried is under its original heading — which is what the plate sheets cite as `scene-prompt-system.md §…` and `portrait-prompt-system.md §…`. Append-only; not cold-read.
...
**THE SCENE TESTS ARE THE AUTHOR'S FAVOURITE PICTURES IN THE FOLDER (s45): *my favourite compositions, colouring, beauty are the images in scene tests.*** Said of `images/scene-tests/`, which are the oldest images here and were made before the plate system existed. **The differences are mechanical and were found by reading the prompt that made the quartet against the blocks this folder writes now.**

**ONE OF THEM IS NOW A RULING AND THE REST ARE NOT (author, s45: *yes for sure; rest unsure if to make a rule*).**

- **RULED: the global colour ban is dead.** *Cool grey and grey-blue with no brown in them* had been written into eleven consecutive plate blocks — **a rule the author had already removed at s42** (*no need to be a rule — case by case we do adjust*), which came back by copying. **Colour is named positively, on the objects that have it**, the way the scene tests do it: *deep umber and warm charcoal*, *ink-black and dark olive*, per garment, banning nothing. `scene-prompt-system.md` §Colour carries it, along with the second half of the cause — the inherited house Colour paragraph is written for portraits and says *the warmth belongs to the skin alone*, which stacked with the ban cooled every plate twice.
- **RECORDED, NOT RULED:** the scene tests also use **one implied wall and no place**, **soft daylight from one side** rather than a named dramatic source, **nothing happening**, and **landscape** for the conversations where every plate so far is portrait. **Each of the plate instructions that differ was earned by a specific failure** — the reserve after a muddy run, the demotion after a wall drew itself stone by stone, the named source after two runs asked distant lights to behave like near ones. **Individually correct, and stacked they make drama where the author wants beauty.** Nothing here is promoted; it is written down so the next session composing a quiet two-figure moment knows there is a proven register for it.
...
**FINDINGS ARE NOT AUTOMATICALLY RULES (author, s42).** *No need to be a rule — case by case we do adjust.* Ruled after two colour rules were written off three runs and the third run's "fault" turned out to be the best picture of the set. **Record what happened on the plate sheet; promote it into a prompt system only when it has failed the same way with the same cause more than once and the correction has been wanted every time.** The same instinct killed a linter's negation checks and it is the folder's standing bias: the systems are for what reliably repeats, and the pictures are judged by looking at them.
...
**AND THE HAND CROPS ARE OVERSUBSCRIBED (author, s45): *we do lots of close ups of hands... chest up shots, creative expressions, i like better.*** Ruled on the first Ch. 13 block, which was a hand and a forearm in the frost with no face in it — and **the note is about the set, not that picture.** `ch03/the-wrist`, `ch04/the-knot`, `ch05/hand-on-the-rock`, `ch11/the-white-stitch` and the rejected tree are all the same crop, and a sixth was composed without anyone counting the first five. **The default is a chest-up with a face doing something; a hand-and-object crop is the exception, and it has to earn itself against this.** The s43 line is unchanged — a faceless candidate is priced, not disqualified — and what moved is which side of that price the folder should be standing on.

**AND THE REPLACEMENT WAS SPECIFIED, NOT JUST THE COMPLAINT:** *same scene of the receipts, but we can actually see more. Hurt but focused expression... something more visual.* **The moment does not change; the crop widens until the face is in it, and the tell is staged where the wider frame cannot lose it.** That is `ch12/the-law-on-the-table`'s solve — put the small thing where it cannot be missed and let the face be finished too — and it is the answer to §Name the winner's own limit, which says a tell below a hand's size in frame cannot carry finish. **A chest-up is not a wider version of a hand crop. It is the hand crop plus the face, stacked.**
...
**And it is the same instinct five times.** `ch09/aurelians-candle`'s face was ruled correct against this folder's reading that it was years too old, `ch09/the-roller-alone`'s drift was recorded and not corrected, `vask.png` was ruled Vask against the brief's own temple-grey, `ch11/the-fan-in-the-rain` was adopted over a face that came back years past its own anchor, and `neris.png` was ruled good against the brief's *handsome and unremarkable*. **The standing bias is one consistent face across the book, over age-accuracy in any single picture.** `kael-9-still` is moot as a plate anchor and stays where it is: a prompt, unrun, recording why a second face was cut.
...
- `images/` — split by kind (s34), because the library outgrew one flat folder:
  ...
  - `images/held/` — **runs kept for a decision (s42).** A picture that came out well and is not the plate its prompt was written for: the beat moved, or the author has not placed it. **Not pages** — THE PLATE FOLLOWS THE PAGE does not bind them, and nothing here is owed a decision. **The filename is the prompt that made it**, so provenance survives the wait. A held image is either adopted into `plates/` under the slug of the beat it actually shows, or it stays here.
...
# ARCHIVED: canon-brief.md

# Canon brief — paste at the top of any prompting conversation

The prompts are written where `00`, `02` and `03` cannot be seen. Every canon error of s33 came from that one blind spot and none from craft. This is the fix.

**This file is a SUMMARY, not canon.** `00`, `02` and `03` govern; if this disagrees with a sheet, the sheet wins and this file is the bug. Re-read it against them whenever appearance canon moves.

---

## Cross-cutting — true of every image
...
- **The medium speckles every face and it is TEXTURE, NOT A MARK** (author ruling, s34; confirmed s40 when a block that said nothing about freckles came back freckled). Do not read it as canon on anybody and do not write negations to fight it.
...
- **A handsome boy, and filthy with it. He is NOT plain** — and *plain* does not render anyway. Village dirt, weather, hair cut by somebody who was not being careful.
...
- **EYES PALE GREY. FREE IN EVERY IMAGE, INCLUDING PLATES (author ruling, s42 — `README.md`).** Render it at full strength: near-white neutral grey, the LIGHTEST VALUE in the face, winning on value rather than saturation. **Still unspent on the PAGE** — that is a prose staging rule and no longer an image one.
- **Marks:** the **burn speckle-receipts, WRIST TO SECOND KNUCKLE** (Ch. 13 — *the Art writing its own receipts*; **added s45 after a plate block wrote them halfway to the elbow and the run drew what it was told**) · blade scar on the **LEFT forearm** (Ch. 6, echoes his father's tally-marks) · long shallow scar on the **outside of the LEFT arm above the elbow** (Ch. 23) · burn on the **LEFT forearm, wrist to elbow** (Ch. 24).
...
- **Long black hair worn loose, well past the shoulders, no ornament.**
- **EMERALD GREEN eyes** — the colour is spent at **reading distance only**, so it lands in close framing.
- **Her stillness WARNS — nothing like Kael's.** In a close-up it lives entirely in the gaze: level, flat, appraising. *Unimpressed and faintly tired. Not vulnerable, not wistful, not searching.*
- Knuckles older than her face; guard-lineage calluses; old pale scar lines on both forearms with a few fresher.
- **TWIN BLADES, BOTH HANDS FULL.** This is her defining trait and the empire's one-handed convention is precisely what she defies. **Never one sword.** A matched pair, plain, well-made, well-maintained.
- **The capital's grey** — disciplined, uniform-adjacent, austere. Her Line does not decorate; the plainness IS the station. (A bronze collar insignia was adopted s33 as canon-by-precedent.)
- She must stay **distinct from Kael at thumbnail size**: hers long, his short; her blades in hand, his bundled on his back; her provenance eleven generations of service, his a shed.

## AELIANA CASSIVARA VESSANTINE

- **Unmatched, radiant, royal beauty** — the most beautiful person in the book, **never in competition with anyone for it.** The plain cast exists to frame her.
- **Not tall.** Slight, sheltered, soft — **a body that has never worked.** No muscle, no weathering.
- **LONG SILVER-WHITE hair — Vessantine house trait, NOT a drain-mark.** Colourless silver, **never blonde, no yellow or gold.** The lightest mass in the picture.
- **PALE BLUE eyes.**
- **Hair LOOSE by default. UP off the neck at the tournament** (Ch. 22).
- **NO ARMOUR IN BOOK ONE.** She is a symbol, not a fighter. Court dress only.
- **The Ch. 22 gown:** a blue so dark it reads black until the light moves on it; **gold at the shoulder AND the hem, in a quantity with no function except to be a quantity** — the excess itself is the point.
...
- **Pale untouched indoor skin.** Freckles are not a trait of hers — but the medium's granulation renders as speckling on every face, and that is **texture, not a mark** (author ruling, s34). Do not fight it, and do not read it as canon on anybody.
...
- **THE PARENTS ARE THE SAME AGE (author ruling, s34).** A relative fact only — both stay *age unpinned* in `03`, and their image files carry no age suffix.
- **IRON-DARK hair, GREYING EARLY AT THE TEMPLES — ahead of his age, and the grey is the dangerous part.** It must read as **diffuse scatter, salt through the dark. NEVER a streak, seam, blaze or band of white.** `02` has two body-marks of magic and one is **white hair = the drain-mark**; Ruthen enters the book as *the master with the seam of white through his dark hair* and Ch. 9 spends a paragraph on forty children not looking at it. An image cannot tell forty-year-old grey from a drain-mark, so the shape is forbidden and only the scatter is allowed.
- **THE IMAGE LIBRARY DOES NOT CARRY THE TEMPLE-GREY, AND THE AUTHOR RULED THE PICTURES CORRECT ANYWAY (s43).** `images/portraits/vask.png` reads a lean, long-haired man in his middle thirties with no grey in him, and every plate anchored on it inherits that face. The ruling came on `plates/ch10.md`'s waterer — *it looks like Vask to me* — and it is a ruling on the pictures: **the reference is not re-cut, the four adopted plates of him stand, and nothing in `03` moves.** The line above still says what `03` says. **Do not re-raise it and do not re-cut on this account.**
...
- **THE REFERENCE READS EARLY THIRTIES AND STRIKING, AND THE AUTHOR RULED IT GOOD (s44).** `images/portraits/neris.png` does not read *handsome and unremarkable in the same breath* — it reads young and arresting — and every plate anchored on it inherits that face. Raised on `plates/ch12.md`'s law on the table and answered *it's good.* **The reference is not re-cut, the lines above still say what `03` says, and nothing moves.** Do not re-raise it. Second instance of the same pattern as `vask.png`.
...
**THE PLATE FOLLOWS THE PAGE.** An image may depict only what its own chapter has already put in front of the reader. An image cannot decline to answer — so where the page is deliberately silent, the plate answers with **framing** (distance, angle, occlusion), never with invention. If a moment cannot be composed without spending something unspent, the moment is wrong.

---

# ARCHIVED: portrait-prompt-system.md
...
# ARCHIVED: scene-prompt-system.md
...
**AND A CLEAN LINT ON A BLOATED BLOCK IS NOW ON RECORD (s45).** `plates/ch14.md`'s one-finger passed the script with no restatement, no abstraction and its caps count in range, and the author read it and said *very bloated.* It was cut from 816 words to 430 with no channel lost. **The script sees repeated phrases; it cannot see a paragraph that says one thing three ways in three different sentences.** Which is the ruling above, arriving as evidence.
