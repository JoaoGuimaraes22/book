...
**The cut.** Name it as a landmark on the body and say what it puts out of frame — *the bottom edge of the picture crosses both boys just below the collarbone, so that no arms, no hands and no legs are in the frame at all.* *Chest-up* and *waist-up* both came back wider. **AND THE SECOND HALF IS THE HALF THAT DOES THE WORK** (s63, six runs in one session): five blocks named the landmark and said nothing about what it excludes — `ch12`, `ch13`, `ch15`, `ch17`, `ch18` — and every one came back wider than written; `ch19/the-knife-hilt-first` added *so that no laps, no knees, no chairs and no floor are in the frame at all* and held first time. One variable, five to one. A cropped figure needs a floor, or it hangs on bare paper:
