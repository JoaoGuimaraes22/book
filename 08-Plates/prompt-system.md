...
**Age in a plate.** The reference carries apparent age and the fourteen-year-old anchor is the lever; no ratio has pulled an age down in a face-filling crop. Where a face has drifted older — the adolescents do, on nearly every plate — the rung goes in with the number in front, *Age: thirteen. The eyes at the vertical midpoint of the head, the jaw softly defined and not sharp* (`prompts/plates/book-one/ch35-the-spotter.md`, which it brought back to fourteen — author). It is not free beside a hard subject: four minor-markers in one paragraph next to iron on a boy's wrists were refused before any image was made (`ch39`), so beside restraint or harm let the reference carry the age.
...
- **A warm source that LIGHTS the picture costs the palette and the bare paper inside the frame, and no wording has moved that. A warm source that lights nothing costs nothing** — the camp's fire is in frame at `prompts/plates/book-two/ch55-behind-her-hand.md`, small and far and touching only the doorway, and the picture stayed cool and kept its paper: the cost comes from what the light does, not from the flame being visible. Every lamp-lit or fire-lit run comes back dense and warm, the tooth a hard reticulated texture and almost no bare paper, where daylight holds the register every time; the ink does hold. Inside the frame it is a design choice and not a prompting one: daylight and the register, or the night and the dense warm one. **The margin may be the part that can be bought back, and the evidence is thin.** *The picture falls away to bare unpainted paper at its edges rather than being painted out to the border* has been written into four fire-lit blocks and all four kept their margin (`prompts/plates/book-two/ch53-her-back-went-straight.md`, `ch53-the-hand-on-the-shoulder.md`, `ch55-level-with-the-boy.md`, `ch55-behind-her-hand.md`). **But the base rate is not zero:** `ch51/come-back` kept its margin with no such clause, and `ch56/nine-strides-by-seven` was painted to the border without one. Four for four with it against one of two without it does not separate the wording from the tool, and nothing here is proven. **The clause is an instrument, not part of the template** (§Instruments): put it in where a night picture actually needs its edge, and leave it out otherwise — which is also how the control gets run.
