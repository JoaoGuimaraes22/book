...
- **The programme is chapter order, two plates per chapter** (s42). The target is the first chapter in sequence without two. Ch. 1–17 have theirs.
- **The `plates/`–`prompts/` split stands** (s45): the sheets hold the argument and are the source; `prompts/` is generated.
- **Pictures that stand as drawn:** `vask.png` carries no temple-grey and `neris.png` reads early thirties — both ruled correct (s43, s44), neither re-cut, `03` unchanged. Do not re-raise.
