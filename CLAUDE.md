...
- `08-Plates/` = THE PLATES (reader-facing chapter illustrations). Test: *does this govern how a plate is rendered, or record what one has depicted?* Split like 06/07: `style-anchor.md` owns the RULE (medium, colour law, composition doctrine — invariant); the anchor and plate sheets own the STATE (what an image has committed to). `index.md` = pure TOC. EVICTION: sheets are accumulators and roll per book with 07's; the style anchor never moves.
