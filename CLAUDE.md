...
- `08-Plates/` = THE PLATES (reader-facing illustrations). Test: *does this govern how an image is generated, or record what one has depicted?* `portrait-prompt-system.md` and `scene-prompt-system.md` own the RULE — the fixed STYLE/BASE blocks, the per-character SUBJECT blocks, and the generation method for portraits, full figures and multi-figure scenes; amended in place, never accumulated. `images/` holds the adopted portraits, named `<character>-<age>[-full]`. `plates/` will hold one sheet per chapter plate. EVICTION: plate sheets are accumulators and roll per book with 07's; the prompt system never moves. **THE LEDGER STILL FOLLOWS THE PAGE HERE:** a SUBJECT block is not canon — 00/02/03 are, and a block that disagrees with a sheet is a bug in the block.
