...
- `08-Plates/` = THE PLATES (reader-facing illustrations). Test: *does this govern how an image is generated, or record what one has depicted?* `portrait-prompt-system.md` and `scene-prompt-system.md` own the RULE — the fixed STYLE/BASE blocks, the per-character SUBJECT blocks, and the generation method for portraits, full figures and multi-figure scenes; amended in place, never accumulated. `images/` holds the adopted portraits, named `<character>-<age>[-full]` — **bare `<character>[-full]` where `03` leaves the age unpinned** (s34: a number in a filename is read as canon by the next session, and Vask's and Neris's ages are deliberately open). `plates/` will hold one sheet per chapter plate. EVICTION: plate sheets are accumulators and roll per book with 07's; the prompt system never moves. **THE LEDGER STILL FOLLOWS THE PAGE HERE:** a SUBJECT block is not canon — 00/02/03 are, and a block that disagrees with a sheet is a bug in the block.
...
- **THE PLATE FOLLOWS THE PAGE.** A plate may depict only what its own chapter has already put in front of the reader — the ledger rule pointed at pictures, and the seal on 00. An image cannot decline to answer, so where the page is deliberately silent the plate answers with FRAMING (distance, angle, occlusion), never with invention; if a moment cannot be composed without spending something unspent, the moment is wrong. Full doctrine: `08-Plates/style-anchor.md`.
